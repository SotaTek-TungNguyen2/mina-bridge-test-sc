...
describe('token test', () => {
    ...
    const userPrivkeyD = Local.testAccounts[1].privateKey
    const userPubkeyD = Local.testAccounts[1].publicKey
    ...
    it('can deploy tokens', async () => {
        ...
        await tx.prove()
        tx.sign([userPrivkeyD, tokenPrivkey])
        await tx.send()
        // tokenZkapp.account.tokenSymbol.assertEquals(Field(0))
    ...
    it('can initialize tokens', async () => {
        const tx = await Mina.transaction(userPubkeyD, () => {
            tokenZkapp.initialize(DECIMALS, SUPPLY_MAX)
        })
        await tx.prove()
        tx.sign([userPrivkeyD, tokenPrivkey])
        await tx.send()
